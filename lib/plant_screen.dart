...
class _PlanetScreenState extends State<PlanetScreen> {
  ...
  /* I'll override this plantData now with an async invoked in the initState */
  // List<List<String>> planetDataHardCodedList = [
  //   [
  ...
  loadData() async{
    ...
    planetDataFromFile = listData;
    // print(planetDataFromFile);
    setState(() {
      print(planetDataFromFile);
      isLoading = false;
    });
  ...
  late List<List<dynamic>> planetDataFromFile;
  @override
  void initState() {
    //
    // PlanetData planetData = PlanetData();
    // planetDataFromFile =  planetData.loadData() as List<List<String>>;
     print('#############################');
    // print(planetDataFromFile);
      ...
      body: Stack(
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                NavBarGradient(text: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][3].toString(),),
                Expanded(
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Text(isLoading == true? 'not yet': planetDataFromFile[argIndex+1][4].toString(),                                style: TextStyle(
                                    color: Colors.white,
                                    fontSize: 18,
                            ...
                            Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Text(isLoading == true? 'not yet': planetDataFromFile[argIndex+1][4],                                style: TextStyle(
                                  color: Colors.white,
                                  fontSize: 18,
                                  fontWeight: FontWeight.w300,
                                  // decoration: TextDecoration.underline,
                            ...
                            RowEntry(leading: 'Distance from sun (km):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][5].toString()),
                            RowEntry(leading: 'Length of Day (hours):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][6].toString()),
                            RowEntry(leading: 'Orbital Period (Earth years):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][7].toString()),
                            RowEntry(leading: 'Radius (km):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][8].toString()),
                            RowEntry(leading: 'Mass (kg):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][9].toString()),
                            RowEntry(leading: 'Gravity (m/s²):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][10].toString()),
                            RowEntry(leading: 'Surface Area (km²):', data: isLoading == true? 'not yet': planetDataFromFile[argIndex+1][11].toString()),
